...
import { BarChart3, X, MoreVertical, Power, Trash2 } from 'lucide-react';
import type { MortalityTable } from '../../types';
import { Card, CardHeader, CardTitle, CardContent, Input, Badge, Button, useToast } from '../../design-system/components';
import { useMortalityTables } from '../admin/hooks/useMortalityTables';
import { MortalityMainChart, MortalityComparisonChart, StatisticsPanel } from '../admin/charts';
...
const TablesAnalysisTab: React.FC<TablesAnalysisTabProps> = ({
  ...
  useEffect(() => {
    ...
    if (showTableMenu) {
      // Usar setTimeout para adicionar o listener após o clique atual
  ...
  const getGenderColor = (gender: string) => {
    if (gender === 'M' || gender === 'male') return '#3B82F6'; // azul
    if (gender === 'F' || gender === 'female') return '#EC4899'; // rosa
    return '#6B7280'; // cinza
  };

  const getUniqueColor = (index: number) => {
    // Paleta padrão do Chart.js - testada e acessível
    const chartJsColors = [
  ...
  const handleDeleteTable = async () => {
    ...
    try {
      ...
      // Selecionar outra tábua se a atual foi excluída
  ...
  // Verificar se há dados para mostrar gráfico de comparação
  ...
  const allComparisonData = showComparisonChart ? [
    {
      ...
      color: getUniqueColor(0) // Tábua principal sempre índice 0
    },
    ...comparisonData.map((item, index) => ({
      ...item,
      color: getUniqueColor(index + 1) // Índices 1, 2, 3...
    }))
  ] : [];
                    ...
                    <div className="relative">
                      ...
                      {showTableMenu && (
                        ...
                        >
                          ...
                          >
                            <Power className={`w-4 h-4 ${findTableById(selectedTableCode)?.is_active ? 'text-orange-500' : 'text-green-500'}`} />
